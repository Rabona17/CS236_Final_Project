...
def pick_task(src,preprocess_single):
    for sample in src:
        # print(sample.keys())
        ...
        if 'layoutanalysis.json' in sample:
            ...
            if json.loads(sample['annotation.json'])['css_files_count']>0 and json.loads(sample['annotation.json'])['downloaded_css_files_count']/json.loads(sample['annotation.json'])['css_files_count']>0.5:
                ...
                # print('yielding')
                yield {'image':image, 'layout':json.loads(sample['layoutanalysis.json']), 'pixel_values': preprocess_single(image)}
            else:
                continue
        else:
            # If the selected task is not in the sample, skip it
            continue
def build_wds_dataset(dataset_name_or_paths, split, preprocess_single):
    s3 = boto3.client('s3')
    ...
    pipeline.append(
        partial(pick_task,preprocess_single=preprocess_single)
    )
    if split=='train':
    ...
    return dataset
def clamp(array, min_value, max_value):
    return np.maximum(min_value, np.minimum(array, max_value))
@torch.no_grad()
def log_validation(test_dataloader, vae, accelerator, weight_dtype, epoch):
    ...
    for tracker in accelerator.trackers:
        ...
        if tracker.name == "tensorboard":
            np_images = clamp(np.stack([np.asarray(img)[0] for img in images])/2+0.5, 0, 1)
...
def main():
    ...
    with accelerator.main_process_first():
        ...
        # train_dataset = build_wds_dataset(urls, 'train',preprocess_single)
        # test_dataset = build_wds_dataset(["downloaded_bytes/wds_debug/CC-MAIN-2022-33/0_0/"], 'test',preprocess_single)
    ...
    test_dataloader = torch.utils.data.DataLoader(
        test_dataset, collate_fn=collate_fn
    )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(vae):
                ...
                if hasattr(vae, 'module'):
                    vae = vae.module#.encode(target).latent_dist
                # else:
